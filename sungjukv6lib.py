...
def loadSungJuk():
    ...
    # 파일에 저장된 성적데이터를 행단위로 읽어 리스트에 저장
    with open('data/sungjuk.dat', encoding='UTF-8') as f:
        data = f.readlines()

    outs = []  # 성적 데이터를 저장하기 위해 리스트 선언
    for d in data:  # 성적 데이터를 한 건씩 읽어와서 
        items = d.strip().split(',')   # strip (쓸데 없는 문자들 다 제거함)
        # dict 형식으로 성적데이터를 재작성
...
def saveSungJuk(sjs):
    ...
    with open('data/sungjuk.dat', 'w', encoding='UTF-8', newline='') as f:
        for sj in sjs:
